...
func Trawl(workerID int, app *App) {
	...
	for {
		...
		response, err := GetHTML(nextUrl)
		if err != nil {
			LogAsJSONString(map[string]any{
				"workerid": workerID,
				"url":      nextUrl,
				"func":     "trawl",
		...
		URLInfo["url"] = nextUrl
		URLInfo["statusCode"] = response.StatusCode
		URLInfo["addedCount"] = added

		// Ensure if the URL has been seen a re-processed
		// to add the count instead of replacing it
		oldURLInfo, loaded := app.ProcessedURLs.LoadOrStore(nextUrl, URLInfo)
		if loaded {
			URLInfo["addedCount"] = oldURLInfo.(map[string]any)["addedCount"].(int) + added
			app.ProcessedURLs.Store(nextUrl, URLInfo)
		}
